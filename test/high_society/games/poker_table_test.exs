defmodule HighSociety.Games.PokerTableTest do
  ...
  describe "sit/4" do
    test "debits the buy-in from the user's balance and seats them" do
      ...
      assert {:ok, view} = PokerTable.sit(@slug, user, 0, 200)
      assert view.seats[0].user_id == user.id
      assert view.seats[0].stack == 200

      assert Accounts.get_user!(user.id).balance == 800
    end
    ...
    test "rejects sitting down without enough balance" do
      user = funded_user(10)
      assert {:error, :insufficient_funds} = PokerTable.sit(@slug, user, 0, 40)
    end

    test "rejects a seat that's already taken" do
      user1 = funded_user(1000)
      user2 = funded_user(1000)
      {:ok, _view} = PokerTable.sit(@slug, user1, 0, 200)
      assert {:error, :seat_taken} = PokerTable.sit(@slug, user2, 0, 200)
    end
    ...
    test "a second player sitting down deals a hand" do
      ...
      user2 = funded_user(1000)
      {:ok, _view} = PokerTable.sit(@slug, user1, 0, 200)
      assert {:ok, view} = PokerTable.sit(@slug, user2, 1, 200)
  ...
  describe "stand/2" do
    test "credits the current stack back to the user's balance" do
      user = funded_user(1000)
      {:ok, _view} = PokerTable.sit(@slug, user, 0, 200)
    ...
    test "folds the player's live hand immediately when standing up mid-hand" do
      ...
      user2 = funded_user(1000)
      {:ok, _view} = PokerTable.sit(@slug, user1, 0, 200)
      {:ok, _view} = PokerTable.sit(@slug, user2, 1, 200)
      ...
      assert updated_balance >= 800 - 200 and updated_balance <= 800 + 200
    end
  ...
  describe "act/4" do
    ...
    test "a legal action updates the table state" do
      ...
      user2 = funded_user(1000)
      {:ok, _view} = PokerTable.sit(@slug, user1, 0, 200)
      {:ok, view} = PokerTable.sit(@slug, user2, 1, 200)
